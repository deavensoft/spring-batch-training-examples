...
@Slf4j
public class E06FlatFileItemReaderConfiguration {

  @StepScope
  @Bean
  FlatFileItemReader flatFileItemReader(
      @Value("#{jobParameters['fileInput']}") String inputFileClassPath){

    FlatFileItemReader<Product> reader = new FlatFileItemReader<>();

    // 1. let reader know where is the file
    ClassPathResource inputResource = new ClassPathResource(inputFileClassPath);
